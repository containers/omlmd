...
def pytest_collection_modifyitems(config, items):
    for item in items:
        ...
        skip_e2e_model_registry = pytest.mark.skip(
            reason="this is an end-to-end test, requires explicit opt-in --e2e-model-registry option to run."
        )
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="opt-in to run tests marked with e2e_model_registry",
    )
